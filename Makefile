...
docker-build: amd64-build
	docker buildx build --build-arg VERSION=$(VERSION) --platform linux/amd64 -t $(DOCKER_NAMESPACE)/$(COMPONENT):$(VERSION) -f ./cmd/$(COMPONENT)/Dockerfile .

.PHONY: docker-build-arm
docker-build-arm: arm64-build
	docker buildx build --build-arg VERSION=$(VERSION) --platform linux/arm64 -t $(DOCKER_NAMESPACE)/$(COMPONENT):$(VERSION) -f ./cmd/$(COMPONENT)/Dockerfile .
